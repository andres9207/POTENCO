...
class SSP {
        ...
        for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
            ...
            for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
                ...
                if ( isset( $column['formatter'] ) ) {
                    $row[ $column['dt'] ] = ($isJoin) ? SSP::utf8_encodeFn(SSP::convert_htmlentities($column['formatter']( $data[$i][ $column['field'] ], $data[$i] ))) : SSP::utf8_encodeFn(SSP::convert_htmlentities($column['formatter']( $data[$i][ $column['db'] ], $data[$i] )));
                }
                else {
                    $row[ $column['dt'] ] = ($isJoin) ? SSP::utf8_encodeFn(SSP::convert_htmlentities($data[$i][ $columns[$j]['field'] ])) : SSP::utf8_encodeFn(SSP::convert_htmlentities($data[$i][ $columns[$j]['db'] ]));
                }
            }
     ...
     *
     */
    static function simple ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '',$with='', $having = '')
    {
        $bindings = array();
...
/*var_dump((array(
    ...
    static function utf8_encodeFn($string)
	{
