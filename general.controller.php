...
class general{
    ...
    {
        global $conectar;
        $conperiodo = mysqli_query($conectar,"select distinct liq_inicio ini, liq_fin fin from tbl_liquidar where liq_tipo = 2 ");
        $numperiodo = mysqli_num_rows($conperiodo);
        $periodos = array();
        for($a=0;$a<$numperiodo;$a++)
        {
            $data = mysqli_fetch_array($conperiodo);
            $ini = $data['ini'];
            $fin = $data['fin'];
            $selected = "";
    ...
    function cargarAnos($min,$max, $sel = "", $orden = "DESC", $tipo = 2)
    {
        global $conectar;
        $maxi =  $max + 1;
        $conano = mysqli_query($conectar,"SELECT DISTINCT date_format(fes_fecha,'%Y') as ano FROM tbl_festivos WHERE date_format(fes_fecha,'%Y') <= ".$maxi." ORDER BY date_format(fes_fecha,'%Y')  ".$orden."");
        $numano = mysqli_num_rows($conano);
        $anos = array();
        //echo "SELECT DISTINCT date_format(fes_fecha,'%Y') as ano FROM tbl_festivos WHERE date_format(fes_fecha,'%Y') <= ".$max." ORDER BY date_format(fes_fecha,'%Y') ".$orden;
        for($a=0;$a<$numano;$a++)
        {
            $data = mysqli_fetch_array($conano);
            $an   = $data['ano'];
            $selected = "";
    ...
    function cargarSemanas($ano= 0,$mes = 1,$min = 0,$max = 0,$sel ="", $orden = "DESC",$tipo = 1,$emp = 0)
    {
        ...
        { 
            for($m=$max;$m>=$min;$m--)
            {
                ...
                //VERIFICAR SI YA SE LEGENERO ESA SEMANA AL EMPLEADO SELECCIONADO

                $classSem = "";
                $subSem = "Sin iniciar";
                if($emp>0)
                {
                    $convs = mysqli_query($conectar, "SELECT jor_clave_int,jor_estado from tbl_jornada j where usu_clave_int = '".$emp."' and jor_semana = '".$sema."'");
                    $numvs = mysqli_num_rows($convs);
                    if($numvs>0)
                    {
                        $datvs = mysqli_fetch_array($convs);
                    ...
                    else 
                    {
                        ...
                        $conini = mysqli_query($conectar, "SELECT jod_clave_int from tbl_jornada_dias WHERE usu_clave_int = '".$emp."' and jod_semana = '".$sema."' and jod_total>0");
                        $numini = mysqli_num_rows($conini);
                        $classSem = ($numini>0)?"bg-danger":"";
                        $subSem = ($numini>0)?"Por Cerrar":"Sin iniciar";
                    }
                }
                

                $conld = mysqli_query($conectar,"SELECT STR_TO_DATE('".$sema." Monday', '%x%v %W') LUN,STR_TO_DATE('".$sema." Sunday', '%x%v %W') DOM ");
                $datld = mysqli_fetch_array($conld);
                $sLunes = $datld['LUN'];
                $sDomingo = $datld['DOM'];
                $subtext = $sLunes." - ".$sDomingo;

                $semanas[] = array("id"=>$sema, "nom"=>$sema,"subtext"=>$subtext,"inicio"=> $sLunes,"fin" => $sDomingo, "classSem"=>$classSem, "subSem"=>$subSem);
                if($tipo==1)
                {  
                    if( $sema==$sel){ $selected = "selected"; }else{ $selected = ""; }
    ...
    {
        global $conectar;
        $con = mysqli_query($conectar,"SELECT obr_clave_int id, concat(obr_nombre, '-',obr_nom_proyecto) nom, obr_cencos cen from tbl_obras WHERE est_clave_int = 1 ");
        $obra = array();
    ...
    {
        global $conectar;
        $con = mysqli_query($conectar,"SELECT est_clave_int id, est_nombre nom from tbl_estados WHERE est_clave_int != 3 ");
        $obra = array();
        while($dat = mysqli_fetch_array($con))
        {
            $id  = $dat['id'];
    ...
    {
        global $conectar;
        $sql = "SELECT tin_clave_int id, tin_nombre nom, tin_suma su from tbl_tipo_novedad WHERE tin_aplica = 2 order by nom asc";
        if($opc==2)
        {
            $sql ="SELECT tin_clave_int id, tin_nombre nom, tin_suma su from tbl_tipo_novedad WHERE tin_aplica = 1";
        }

        if($opc==3)
        {
            $sql ="SELECT tin_clave_int id, tin_nombre nom, tin_suma su from tbl_tipo_novedad ";
        }
    ...
    {
        global $conectar;
        $con = mysqli_query($conectar,"SELECT u.usu_clave_int id, concat(usu_apellido,' ',usu_nombre) nom, usu_correo cor from tbl_usuarios u WHERE (u.est_clave_int = 1) or usu_clave_int = '".$sel."' order by usu_apellido");
        $empleados = array();
        while($dat = mysqli_fetch_array($con))
        {
            $id  = $dat['id'];
    ...
    {
        global $conectar;
        $con = mysqli_query($conectar,"SELECT distinct liq_inicio, liq_fin from tbl_liquidar order by liq_inicio desc");
        $periodos = array();
        while($dat = mysqli_fetch_array($con))
        {
            $inicio = $dat['liq_inicio'];
            $fin    = $dat['liq_fin'];
            $text = "".$inicio." a ".$fin;
            $selected = "";
            if($id==$text){ $selected = "selected"; }
            if($tip==1)
            {
    ...
    {
        global $conectar;
        $con = mysqli_query($conectar,"SELECT hor_clave_int id, hor_nombre nom from tbl_horarios WHERE est_clave_int = 1 or hor_clave_int = '".$sel."'");
        $horarios = array();
        while($dat = mysqli_fetch_array($con))
        {
            $id  = $dat['id'];
    ...
    {
        global $conectar;
        $con = mysqli_query($conectar, "SELECT hor_clave_int, hor_nombre,hor_1, hor_2, hor_3, hor_4, hor_5, hor_6, hor_7, hor_8 from tbl_horarios where hor_clave_int = '".$id."'");
    ...
    {
        global $conectar;
        $con = mysqli_query($conectar, "SELECT obr_clave_int, obr_nombre,obr_encargado, obr_vr_operador,obr_vr_maquina,obr_hr_mes,obr_cencos,obr_lunes, obr_martes, obr_miercoles, obr_jueves, obr_viernes, obr_sabado, obr_domingo, obr_vr_senalero, obr_hr_semana, est_clave_int, obr_auxilio, obr_vr_auxilio, obr_nom_proyecto,obr_fec_inicio,obr_ubicacion, obr_vr_elevador, obr_operador, obr_senalero, obr_contrato, obr_festivo from tbl_obras where obr_clave_int = '".$id."'");
        $dat = mysqli_fetch_array($con);
        return $dat;
    }
    ...
    {
        global $conectar;
        $conreglas = mysqli_query($conectar, "SELECT calculoHoras3('".$fecha."','".$hi."','".$hf."') ho");
        $datr = mysqli_fetch_array($conreglas);
        mysqli_free_result($conreglas);
        return $datr['ho'];
    }
    ...
    function fnExtras($tip, $G5, $F5,$Q5,$R5,$M5, $O5, $L5,$AA5, $I5, $F6, $G6, $P5, $T5, $S5,$Y5, $AC5, $U5, $X5, $Z5, $AD5, $HIRN, $HFRN, $thorario = 8)//HIRN HORA INICIAL REGARGO NOCTURNO
    {
       ...
       // 0,04166666666666666666666666666667 1AM
        ...
        if($tip==7)
        {
            ...
            ((($G6==1 || $F6==1) and $AA5>0 and ($G5<>1 and $F5<>1))?$O5*24:0)-//
            ((($G5==1 || $F5==1) and $S5=="SI" and $T5=="")? ($Q5<=$thorario?$Q5:$thorario):0)-
            (($F6<>1 and $G6<>1 and ($F5==1 || $G5==1) and $O5<($HFRN))?$O5*24:0)-$AC5;
    ...
    function getPorcentaje($tipo, $opc = 1, $id = 0)//1 normal 2 segun liquidacion aprobada, 3 segun liquidacion de obra $id = segun liquidacion o segun liquidacion obra
    {
        global $conectar;
        if($opc==1)
        {
            $sql =  "SELECT hor_porcentaje, hor_codigo, hor_descripcion FROM tbl_horas WHERE hor_nombre = '".$tipo."' LIMIT 1";
        }
        else if($opc==2){
            $sql =  "SELECT lih_porcentaje, hor_codigo, hor_descripcion FROM tbl_horas h join tbl_liquidar_horas l on l.hor_clave_int = h.hor_clave_int WHERE hor_nombre = '".$tipo."' and l.liq_clave_int = '".$id."' LIMIT 1";
        }
        else if($opc==3){
            $sql =  "SELECT lih_porcentaje, hor_codigo, hor_descripcion FROM tbl_horas h join tbl_liquidar_obras_horas l on l.hor_clave_int = h.hor_clave_int WHERE hor_nombre = '".$tipo."' and l.lio_clave_int = '".$id."' LIMIT 1";
        }
        $con = mysqli_query($conectar,$sql);
        $dat = mysqli_fetch_array($con);
        return $dat;
        /*$por = $dat['hor_porcentaje']; $por = ($por=="" || $por==NULL)?0:$por;
        mysqli_free_result($con);
        return $por;*/
    }
    ...
    {
        global $conectar;
        $con = mysqli_query($conectar,"SELECT jod_fecha fec, usu_clave_int emp FROM tbl_jornada_dias WHERE jod_clave_int = '".$idfec."'");
        $dat = mysqli_fetch_array($con);
        return $dat;        
    }
    ...
    {
        global $conectar;
        $con = mysqli_query($conectar,"SELECT obr_clave_int obr FROM tbl_jornada_horas WHERE joh_clave_int = '".$id."'");
        $dat = mysqli_fetch_array($con);
        return $dat;        
    } 
}
