...
class general {
    ...
    function calcularDiaLiquidar($emp,$fecj, $usuario, $fecha)
    {
        ...
        $sqlUsu = "SELECT hor_clave_int, usu_auxilio FROM tbl_usuarios WHERE usu_clave_int = :emp LIMIT 1";
        $stmt = $conn->prepare($sqlUsu);
        $stmt->bindParam(':emp', $emp, PDO::PARAM_INT); // Asegúrate que $emp es un número. Si es string, cambia a PARAM_STR
        $stmt->execute();        
        $dat = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        WHERE jd.jod_fecha = :fecj AND jd.usu_clave_int = :emp
        ORDER BY jod_fecha ASC
        LIMIT 1";

        $stmt = $conn->prepare($sqldias);
        $stmt->bindParam(':fecj', $fecj, PDO::PARAM_STR); // Si $fecj es 'YYYY-MM-DD'
        $stmt->bindParam(':emp', $emp, PDO::PARAM_INT);   // Asegúrate que $emp es numérico
        $stmt->execute();
        $datd = $stmt->fetch(PDO::FETCH_ASSOC);
        $numdias = count($datd);
        ...
        $sqlbon = "SELECT 
        concatLabores(:emp, :fec, '') AS labores, 
        conBonificacion(:emp, :fec, '') AS bon";
        $stmt = $conn->prepare($sqlbon);
        $stmt->bindParam(':emp', $emp, PDO::PARAM_INT); // o PARAM_STR si el valor es alfanumérico
        $stmt->bindParam(':fec', $fec, PDO::PARAM_STR); // formato 'YYYY-MM-DD'
        $stmt->execute();
        $datbon = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        {           
            ...
            //total por permisos
            $sqlper = "SELECT SUM(calculoHoras3(jd.jod_fecha, joh_inicio, joh_fin)) AS totalpermisos 
            FROM tbl_jornada_dias jd 
            ...
            WHERE tn.tin_suma = 6 AND jd.jod_clave_int = :idfec 
            GROUP BY jd.jod_clave_int";

            $stmt = $conn->prepare($sqlper);
            $stmt->bindParam(':idfec', $idfec, PDO::PARAM_INT);
            $stmt->execute();
            $datper = $stmt->fetch(PDO::FETCH_ASSOC);
            $totalpermiso = $datper['totalpermisos'] ?? 0;
    
    
    
            //VERIFICAR SI LA HORA INICIAL del dia es pm o am en caso de que sea pm no hay horari am
            $sqlam = "SELECT joh_clave_int, joh_inicio AS ini, joh_fin AS fin, TIME_FORMAT(joh_inicio, '%p') AS dig 
            FROM tbl_jornada_horas jh 
            JOIN tbl_tipo_novedad t ON t.tin_clave_int = jh.tin_clave_int 
            WHERE t.tin_tipo = 1 AND jod_clave_int = :idfec 
            ORDER BY joh_clave_int ASC 
            LIMIT 1";

            $stmt = $conn->prepare($sqlam);
            $stmt->bindParam(':idfec', $idfec, PDO::PARAM_INT);
            $stmt->execute();
            $datam = $stmt->fetch(PDO::FETCH_ASSOC);
            ...
            $sqlminpm = "SELECT MIN(joh_fin) AS fin, MAX(joh_fin) AS maxfin, TIME_FORMAT(joh_fin, '%p') AS dig 
            FROM tbl_jornada_horas jh 
            JOIN tbl_tipo_novedad t ON t.tin_clave_int = jh.tin_clave_int 
            WHERE t.tin_tipo = 1 AND TIME_FORMAT(joh_fin, '%p') = 'PM' 
            AND jod_clave_int = :idfec";

            $stmt = $conn->prepare($sqlminpm);
            $stmt->bindParam(':idfec', $idfec, PDO::PARAM_INT);
            $stmt->execute();
            $datpm = $stmt->fetch(PDO::FETCH_ASSOC);
            ...
            // CON HORA DESCANSO
            $sqldes = "SELECT joh_inicio AS ini, joh_fin AS fin 
            FROM tbl_jornada_horas jh 
            JOIN tbl_tipo_novedad t ON t.tin_clave_int = jh.tin_clave_int 
            WHERE t.tin_suma = 2 AND jod_clave_int = :idfec 
            ORDER BY joh_clave_int ASC 
            LIMIT 1";

            $stmt = $conn->prepare($sqldes);
            $stmt->bindParam(':idfec', $idfec, PDO::PARAM_INT);
            $stmt->execute();
            $datdes = $stmt->fetch(PDO::FETCH_ASSOC);
            $hides = $datdes['ini'];
            $hfdes = $datdes['fin'];
  
    
            //MINIMO AM SI EXISTIERA
            $sqlminam = "SELECT MIN(joh_inicio) AS ini, MAX(joh_fin) AS ammax 
            FROM tbl_jornada_horas jh 
            JOIN tbl_tipo_novedad t ON t.tin_clave_int = jh.tin_clave_int 
            WHERE t.tin_tipo = 1 AND TIME_FORMAT(joh_inicio, '%p') = 'AM' 
            AND jod_clave_int = :idfec";

            $stmt = $conn->prepare($sqlminam);
            $stmt->bindParam(':idfec', $idfec, PDO::PARAM_INT);
            $stmt->execute();
            $datam = $stmt->fetch(PDO::FETCH_ASSOC);

            $aminimin = $datam['ini'];
            $amfinmax = $datam['ammax'];

    
            //PRIMER HORARIO PM
            $sqlpm = "SELECT MIN(joh_inicio) AS fin, TIME_FORMAT(joh_inicio, '%p') AS dig 
            FROM tbl_jornada_horas jh 
            JOIN tbl_tipo_novedad t ON t.tin_clave_int = jh.tin_clave_int 
            WHERE t.tin_tipo = 1 AND TIME_FORMAT(joh_inicio, '%p') = 'PM' 
            AND jod_clave_int = :idfec";
            ...
            $sqlhf = "SELECT joh_clave_int, joh_fin AS fin, TIME_FORMAT(joh_fin, '%p') AS dig 
            FROM tbl_jornada_horas jh 
            JOIN tbl_tipo_novedad t ON t.tin_clave_int = jh.tin_clave_int 
            WHERE t.tin_tipo = 1 AND jod_clave_int = :idfec 
            ORDER BY joh_clave_int DESC 
            LIMIT 1";

            $stmt = $conn->prepare($sqlhf);
            $stmt->bindParam(':idfec', $idfec, PDO::PARAM_INT);
            $stmt->execute();
            $datpmf = $stmt->fetch(PDO::FETCH_ASSOC);
            ...
            $RDT  = self::fnExtrasText(5, $G5, $F5,$Q5,$R5,$M5, $O5, $L5,$AA5, $I5, $F6, $G6, $P5, $T5, $S5,$Y5, $AC5, $U5, $X5, $Z5, $AD5, $tshirn, $tshfrn, $thorario);
            $RD   = self::fnExtras(5, $G5, $F5,$Q5,$R5,$M5, $O5, $L5,$AA5, $I5, $F6, $G6, $P5, $T5, $S5,$Y5, $AC5, $U5, $X5, $Z5, $AD5, $tshirn, $tshfrn, $thorario);
            $AD5  = $RD;
            // echo $RDT."---".$RD."---".$thorario;
            ...
            $sqlvliq = "SELECT liq_clave_int 
            FROM tbl_liquidar 
            WHERE usu_clave_int = :emp 
            AND obr_clave_int <= 0 
            AND :fec BETWEEN liq_inicio AND liq_fin 
            LIMIT 1";

            $stmt = $conn->prepare($sqlvliq);
            $stmt->bindParam(':emp', $emp, PDO::PARAM_INT);
            $stmt->bindParam(':fec', $fec, PDO::PARAM_STR);
            $stmt->execute();
            $datvliq = $stmt->fetch(PDO::FETCH_ASSOC);

            $idliq = $datvliq ? $datvliq['liq_clave_int'] : 0;
            ...
            $sql = "SELECT lid_clave_int, lid_auxilio, liq_clave_int 
            FROM tbl_liquidar_dias 
            WHERE usu_clave_int = :emp AND lid_fecha = :fec";
            ...
            $iddia = $datdia ? $datdia['lid_clave_int'] : 0;
            ...
            if($iddia >0)
            {
                ...
                $sqlupdfec = "UPDATE tbl_liquidar_dias 
                SET lid_fecha = :fec, lid_horario = :horario, lid_hi_man = :aini, lid_hf_man = :afin, 
                lid_hi_tar = :pini, lid_hf_tar = :pfin, lid_horas = :tothoras, lid_hedo = :HEDO, 
                lid_hdf = :HDF, lid_hedf = :HEDF, lid_heno = :HENO, lid_henf = :HENF, lid_rn = :RN, 
                lid_hnf = :HNF, lid_rd = :RD, lid_usu_actualiz = :usuario, lid_fec_actualiz = :fecha, 
                lid_alimentacion = :ali, lid_val_alimentacion = :valorali, lid_compensado = :S5, 
                lid_remunerado = :U5, lid_observacion = :obs, tin_clave_int = :idnov, 
                lid_labores = :labores, lid_bonificacion = :bonificacion, lid_permisos = :totalpermiso, 
                liq_clave_int = :idliq 
                WHERE lid_clave_int = :iddia";

                $stmt = $conn->prepare($sqlupdfec);
                $stmt->execute([
                ':fec' => $fec,
                ':horario' => $horario,
                ':aini' => $aini,
                ...
                ':totalpermiso' => $totalpermiso,
                ':idliq' => $idliq,
                ':iddia' => $iddia
                ]);

                $msn = $stmt->rowCount() > 0 
                ? "Día actualizado fec:$fec - afin:$afin. pminimin:$pminimin hfini:$hfini - hides:$hides aminimin:$aminimin pini:$pini - pfin:$pfin. datpmfin:".$datpm['fin']." ".$hffin.".- Liquidación: $idliq Q5=$Q5 THORARIO: $thorario" 
                : "No actualizó día $fec";
            }
            else
            {
                ...
                // }   
                $sqlinsfec = "INSERT INTO tbl_liquidar_dias (
                    lid_fecha, lid_horario, lid_hi_man, lid_hf_man, lid_hi_tar, lid_hf_tar, 
                    lid_horas, lid_hedo, lid_hdf, lid_hedf, lid_heno, lid_henf, lid_rn, lid_hnf, lid_rd, 
                    usu_clave_int, lid_usu_actualiz, lid_fec_actualiz, lid_compensado, lid_remunerado, 
                    lid_observacion, tin_clave_int, lid_labores, lid_bonificacion, lid_auxilio, 
                    lid_permisos, liq_clave_int
                ) VALUES (
                    :fec, :horario, :aini, :afin, :pini, :pfin, :tothoras, :HEDO, :HDF, :HEDF, 
                    :HENO, :HENF, :RN, :HNF, :RD, :emp, :usuario, :fecha, :S5, :U5, :obs, 
                    :idnov, :labores, :bonificacion, :auxilio, :totalpermiso, :idliq
                )";
    
                $stmt = $conn->prepare($sqlinsfec);
                ...
                if ($success) {
                    $idfec = $conn->lastInsertId();
                } else {
                    $msn = "No insertó día $fec";
                }                     
            }
        }
        else
        {
            ...
            if($numdias>0)
            {
                ...
                if ($sumnov >= 3) {
                    ...
                    $U5 = ($sumnov == 5) ? "SI" : "";
                    $tothoras = (
                        in_array($sumnov, [4, 5, 7]) && $tothoras <= $thorario && $fes != 1
                    ) ? $thorario : $tothoras;
            
                    if ($totalpermiso <= 0 && $sumnov == 6) {
                        $totalpermiso = $thorario;
                    }
                }       
                ...
                WHERE usu_clave_int = :emp AND lid_fecha = :fec";
                $stmt = $conn->prepare($sql);
                $stmt->execute([':emp' => $emp, ':fec' => $fec]);
                $datdia = $stmt->fetch(PDO::FETCH_ASSOC);

                if ($datdia) {
                    // Día existente → UPDATE
                    $iddia = $datdia['lid_clave_int'];
                    $idliq = ($datdia['liq_clave_int'] > 0) ? $datdia['liq_clave_int'] : $idliq;
                    $aux   = $datdia['lid_auxilio'];

                    $sqlUpdate = "UPDATE tbl_liquidar_dias 
                            SET lid_fecha = :fec, lid_horario = :horario,
                                lid_hi_man = '00:00:00', lid_hf_man = '00:00:00',
                                lid_hi_tar = '00:00:00', lid_hf_tar = '00:00:00',
                                lid_horas = :tothoras, lid_hedo = '0', lid_hdf = '0',
                                lid_hedf = '0', lid_heno = '0', lid_henf = '0',
                                lid_rn = '0', lid_hnf = '0', lid_rd = '0',
                                lid_usu_actualiz = :usuario, lid_fec_actualiz = :fecha,
                                lid_alimentacion = '0', lid_val_alimentacion = '0',
                                lid_compensado = :S5, lid_remunerado = :U5,
                                lid_observacion = :obs, tin_clave_int = :idnov,
                                lid_labores = :labores, lid_bonificacion = :bonificacion,
                                lid_permisos = :totalpermiso, liq_clave_int = :idliq 
                            WHERE lid_clave_int = :iddia";
                    ...
                    $msn = $stmt->rowCount() > 0 
                    ? "Día actualizado fec: $fec - Liquidación: $idliq" 
                    : "No actualizó día n: $fec";
                } else {
                    // Día no existe → INSERT
                    $sqlInsert = "INSERT INTO tbl_liquidar_dias (
                                lid_fecha, lid_horario, lid_hi_man, lid_hf_man,
                                lid_hi_tar, lid_hf_tar, lid_horas, lid_hedo,
                                lid_hdf, lid_hedf, lid_heno, lid_henf, lid_rn,
                                lid_hnf, lid_rd, usu_clave_int, lid_usu_actualiz,
                                lid_fec_actualiz, lid_compensado, lid_remunerado,
                                lid_observacion, tin_clave_int, lid_labores,
                                lid_bonificacion, lid_auxilio, lid_permisos, liq_clave_int
                            ) VALUES (
                                :fec, :horario, '00:00:00', '00:00:00',
                                '00:00:00', '00:00:00', :tothoras, '0',
                                '0', '0', '0', '0', '0',
                                '0', '0', :emp, :usuario,
                                :fecha, :S5, :U5,
                                :obs, :idnov, :labores,
                                :bonificacion, '0', :totalpermiso, :idliq
                            )";
            ...
            {       
                ...
                // }
                 // No hay jornada → registrar mensaje o eliminar si existía
                $msn = "No hay jornada en ese día numdias: $numdias tothoras:$tothoras";

                $sqlCheck = "SELECT lid_clave_int FROM tbl_liquidar_dias 
                            WHERE usu_clave_int = :emp AND lid_fecha = :fec";
                $stmt = $conn->prepare($sqlCheck);
                $stmt->execute([':emp' => $emp, ':fec' => $fec]);
                
                if ($stmt->rowCount() > 0) {
                    // (OPCIONAL) Si quisieras eliminar o limpiar el registro
                    $sqlDel = "UPDATE tbl_liquidar_dias 
                            SET lid_horas = 0 
                            WHERE usu_clave_int = :emp AND lid_fecha = :fec";
    ...
    function calcularDiaObra($emp,$fecj, $obr, $usuario, $fecha)
    {
        ...
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':fecj', $fecj, PDO::PARAM_STR); // fecha 'YYYY-MM-DD'
        $stmt->bindParam(':emp', $emp, PDO::PARAM_INT);
        $stmt->execute();
        $datd = $stmt->fetch(PDO::FETCH_ASSOC); // Si existe, aquí obtienes toda la info  
        ...
        FROM tbl_jornada_horas jh
        JOIN tbl_tipo_novedad t ON t.tin_clave_int = jh.tin_clave_int
        WHERE t.tin_suma = 2 AND jod_clave_int = :idfec AND jh.obr_clave_int = :obr
        ORDER BY joh_clave_int ASC LIMIT 1";

        $stmt = $conn->prepare($sqldes);
        $stmt->execute([':idfec' => $idfec, ':obr' => $obr]);
        $datdes = $stmt->fetch(PDO::FETCH_ASSOC);
        $hides = $datdes['ini'];
        $hfdes = $datdes['fin'];

        //VERIFICAR SI LA HORA INICIAL del dia es pm o am en caso de que sea pm no hay horari am      
        $sqlamini = "SELECT joh_clave_int, joh_inicio AS ini, TIME_FORMAT(joh_inicio, '%p') AS dig
        FROM tbl_jornada_horas jh
        JOIN tbl_tipo_novedad t ON t.tin_clave_int = jh.tin_clave_int
        WHERE t.tin_tipo = 1 AND jod_clave_int = :idfec AND jh.obr_clave_int = :obr
        ORDER BY joh_clave_int ASC LIMIT 1";
        $stmt = $conn->prepare($sqlamini);
        $stmt->execute([':idfec' => $idfec, ':obr' => $obr]);
        $datam = $stmt->fetch(PDO::FETCH_ASSOC);
        $amini = $datam['ini'];
        $dig   = $datam['dig'];
        ...
        $sqlminam = "SELECT MIN(joh_inicio) AS ini
        FROM tbl_jornada_horas jh
        JOIN tbl_tipo_novedad t ON t.tin_clave_int = jh.tin_clave_int
        WHERE t.tin_tipo = 1 AND TIME_FORMAT(joh_inicio, '%p') = 'AM'
        AND jod_clave_int = :idfec AND jh.obr_clave_int = :obr";
        $stmt = $conn->prepare($sqlminam);
        $stmt->execute([':idfec' => $idfec, ':obr' => $obr]);
        $datam = $stmt->fetch(PDO::FETCH_ASSOC);
        $aminimin = $datam['ini'];
        ...
        //Primer horario PM
        $sqlphpm = "SELECT MIN(joh_inicio) AS fin, TIME_FORMAT(joh_inicio, '%p') AS dig
        FROM tbl_jornada_horas jh
        JOIN tbl_tipo_novedad t ON t.tin_clave_int = jh.tin_clave_int
        WHERE t.tin_tipo = 1 AND TIME_FORMAT(joh_inicio, '%p') = 'PM'
        AND jod_clave_int = :idfec AND jh.obr_clave_int = :obr";
        $stmt = $conn->prepare($sqlphpm);
        $stmt->execute([':idfec' => $idfec, ':obr' => $obr]);
        $datpm = $stmt->fetch(PDO::FETCH_ASSOC);
        $pminimin = $datpm['fin'];
        $pmdig    = $datpm['dig'];
        ...
        //HORA FINAL REGISTRADA
        $sqlpmf = "SELECT joh_clave_int, joh_fin AS fin
        FROM tbl_jornada_horas jh
        JOIN tbl_tipo_novedad t ON t.tin_clave_int = jh.tin_clave_int
        WHERE t.tin_tipo = 1 AND jod_clave_int = :idfec AND jh.obr_clave_int = :obr
        ORDER BY joh_clave_int DESC LIMIT 1";

        $stmt = $conn->prepare($sqlpmf);
        $stmt->execute([':idfec' => $idfec, ':obr' => $obr]);
        $datpmf = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        $afin = $dig === "PM" ? "" : (
        strtotime($datpm['fin']) > strtotime("12:00") 
        ? "12:00" 
        : $datpm['fin']
        );
        ...
        $sqlper = "SELECT 
        SUM(calculoHoras3(jd.jod_fecha, joh_inicio, joh_fin)) AS totalpermisos 
        FROM tbl_jornada_dias jd 
        JOIN tbl_jornada_horas jh ON jh.jod_clave_int = jd.jod_clave_int 
        JOIN tbl_tipo_novedad tn ON tn.tin_clave_int = jh.tin_clave_int 
        WHERE tn.tin_suma = 6 
        AND jd.jod_clave_int = :idfec 
        AND jh.obr_clave_int = :obr 
        GROUP BY jd.jod_clave_int";
        ...
        //  Paso 1: Obtener labores y bonificación
        $sqlbon = "SELECT 
        concatLabores(:emp, :fec, :obr) AS labores,
        conBonificacion(:emp, :fec, :obr) AS bon";
        ...
        //aso 2: Verificar si existe liquidación para ese día y obra
        $sqlliq = "SELECT liq_clave_int 
        FROM tbl_liquidar 
        WHERE usu_clave_int = :emp 
        ...
        $sqlvdia = "SELECT lid_clave_int, lid_estado, liq_clave_int 
        FROM tbl_liquidar_dias_obra 
        WHERE usu_clave_int = :emp 
        AND lid_fecha = :fec 
        AND obr_clave_int = :obr";

        $stmt = $conn->prepare($sqlvdia);
        $stmt->execute([
            ':emp' => $emp,
            ':fec' => $fec,
            ':obr' => $obr
        ...
        if($datdia)
        {
            ...
            $idliq = ($datdia['liq_clave_int'] > 0) ? $datdia['liq_clave_int'] : $idliq;
            //$totald = $datfec['jod_total'];
            $sqlupdfec = "UPDATE tbl_liquidar_dias_obra SET 
            lid_fecha = :fec, lid_horario = :horario, 
            lid_hi_man = :aini, lid_hf_man = :afin, 
            lid_hi_tar = :pini, lid_hf_tar = :pfin, 
            lid_horas = :tothoras, lid_hedo = :HEDO, lid_hdf = :HDF, 
            lid_hedf = :HEDF, lid_heno = :HENO, lid_henf = :HENF, 
            lid_rn = :RN, lid_hnf = :HNF, lid_rd = :RD, 
            lid_usu_actualiz = :usuario, lid_fec_actualiz = :fecha, 
            lid_alimentacion = :ali, lid_val_alimentacion = :valorali, 
            lid_compensado = :S5, lid_remunerado = :U5, 
            lid_observacion = :obs, tin_clave_int = :idnov, 
            lid_horas_maquina = :tothorasmaquina, lid_labores = :labores, 
            lid_bonificacion = :bonificacion, lid_permisos = :totalpermiso, 
            liq_clave_int = :idliq 
        WHERE lid_clave_int = :iddia";
            ...
            $success = $stmt->execute([
                ...
                ':obs' => $obs,
                ':idnov' => $idnov,
                ':tothorasmaquina' => $tothorasmaquina,
                ':labores' => $labores,
                ':bonificacion' => $bonificacion,
                ':totalpermiso' => $totalpermiso,
                ':idliq' => $idliq,
            ...
            $msn .= $success ? " Se actualizó día obra $fec" : " No actualizó día obra $fec";         
        }
        else
        {
            $sqlinsfec = "INSERT INTO tbl_liquidar_dias_obra (
                lid_fecha, lid_horario, lid_hi_man, lid_hf_man,
                lid_hi_tar, lid_hf_tar, lid_horas, lid_hedo, lid_hdf,
                lid_hedf, lid_heno, lid_henf, lid_rn, lid_hnf, lid_rd,
                usu_clave_int, lid_usu_actualiz, lid_fec_actualiz, obr_clave_int,
                lid_compensado, lid_remunerado, lid_observacion, tin_clave_int,
                lid_horas_maquina, lid_labores, lid_bonificacion, lid_auxilio,
                lid_permisos, liq_clave_int
            ) VALUES (
                :fec, :horario, :aini, :afin,
                :pini, :pfin, :tothoras, :HEDO, :HDF,
                :HEDF, :HENO, :HENF, :RN, :HNF, :RD,
                :emp, :usuario, :fecha, :obr,
                :S5, :U5, :obs, :idnov,
                :tothorasmaquina, :labores, :bonificacion, :auxilio,
                :totalpermiso, :idliq
            )";
    ...
    function getPorcentaje( $tipo, $opc = 1, $id = 0) // $pdo es el objeto PDO previamente creado
    {
        global $conn;
        switch ($opc) {
            case 1:
