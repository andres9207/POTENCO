...
if($opcion=="NUEVO" || $opcion=="EDITAR" || $opcion=="APPROVED" || $opcion=="EDITARJORNADA")
{
    ...
    $stmt = $conn->prepare("SELECT j.usu_clave_int AS id, j.jor_ano AS ano, j.jor_semana AS sem 
    FROM tbl_jornada j 
    WHERE j.jor_clave_int = :id");
    $stmt->execute([':id' => $id]);
...
else if($opcion=="CARGARSEMANA")
{
    ...
    else
    {
       ...
       // Consulta de jornada por semana y usuario
        $stmt = $conn->prepare("SELECT jor_clave_int, jor_semana, jor_estado, jor_nota 
        FROM tbl_jornada 
        WHERE usu_clave_int = :emp AND jor_semana = :sem");
        $stmt->execute([':emp' => $emp, ':sem' => $sem]);
        $numsem = $stmt->rowCount();
        $datsem = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        $visbot = ($estjor == 1 && $p99 <= 0) ? "hide" : "";
        $disinp = ($estjor == 1 && $p99 <= 0) ? "disabled" : "";

        // Consulta de total de horas
        $stmt2 = $conn->prepare("
        SELECT SUM(calculoHoras3(jd.jod_fecha, joh_inicio, joh_fin)) AS totalhoras 
        ...
        WHERE tn.tin_tipo = 1 AND jd.usu_clave_int = :emp AND jd.jod_semana = :sem
        ");
        $stmt2->execute([':emp' => $emp, ':sem' => $sem]);
        $dattot = $stmt2->fetch(PDO::FETCH_ASSOC);

        $totalhoras = $dattot['totalhoras'];
        $totalhoras = (empty($totalhoras)) ? 0 : $totalhoras;
        ...
        <!--<thead>
            ...
            <tbody>
                ...
                for ($i = $fechaInicio; $i <= $fechaFin; $i += 86400) {
                    ...
                    $fec = date("Y-m-d", $i);
                    $dia = strftimeCustom($fec);
                    $ids = strtotime($fec);
                
                    // Verificación si el día es festivo
                    $stmtFest = $conn->prepare("SELECT diaHabil(:fecha) AS di");
                    $stmtFest->execute([':fecha' => $fec]);
                    $datfec = $stmtFest->fetch(PDO::FETCH_ASSOC);
                    $fes = $datfec['di'];
                    $bgfes = ($fes == 1) ? "bg-primary" : "";
                    $titfes = ($fes == 1) ? "Día Festivo" : "";
                
                    // Verificar si ya existe el día en tbl_jornada_dias
                    $stmtVeri = $conn->prepare("SELECT jod_clave_int, jod_total, tin_clave_int 
                                                FROM tbl_jornada_dias 
                                                WHERE usu_clave_int = :emp AND jod_semana = :sem AND jod_fecha = :fec");
                    $stmtVeri->execute([':emp' => $emp, ':sem' => $sem, ':fec' => $fec]);
                    $numv = $stmtVeri->rowCount();
                    ...
                    if ($numv > 0) {
                        $datfec = $stmtVeri->fetch(PDO::FETCH_ASSOC);
                        $idfec = $datfec['jod_clave_int'];
                        $totald = $datfec['jod_total'];
                        $nov = $datfec['tin_clave_int'];
                    } else {
                        // Insertar el día si no existe
                        $stmtIns = $conn->prepare("INSERT INTO tbl_jornada_dias (jod_fecha, usu_clave_int, jod_semana, jod_usu_actualiz, jod_fec_actualiz) 
                                                   VALUES (:fec, :emp, :sem, :usuario, :fecha)");
                        ...
                        if ($success) {
                            ...
                            echo "<tr><td colspan='2'>No se insertó el día $fec</td></tr>";
                        }
                    }
                    ...
                    if($idfec>0)
                    {
                        ...
                        <tr class="<?php if(strtotime($fec)>strtotime(date('Y-m-d'))){ echo "hide"; }?>">
                            <td id="tdfec<?php echo $idfec;?>" colspan="3" class="pt-0 pb-1 pl-0 pr-0">
                                <?php
                               $stmtHor = $conn->prepare("SELECT joh_clave_int AS id, obr_clave_int AS obr, 
                               DATE_FORMAT(joh_inicio,'%H:%i') AS ini, 
                               DATE_FORMAT(joh_fin,'%H:%i') AS fin, 
                               tin_clave_int AS tin, joh_observacion AS obs 
                               FROM tbl_jornada_horas 
                               WHERE jod_clave_int = :idfec");
                                    $stmtHor->execute([':idfec' => $idfec]);
                                    $horas = $stmtHor->fetchAll(PDO::FETCH_ASSOC);
                                    $numhor = count($horas);
                                    
                                    if ($numhor <= 0) {
                                        echo "<div class='alert alert-light text-center m-0'>No hay Horarios Laborados</div>";
                                    } else {
                                        ?>
                                        ...
                                        $hori = array();
                                        $hori[] = "06:00";
                                        foreach ($horas as $index => $dath) {
                                            $joh = $dath['id'];
                                            $obr = $dath['obr'];
                                            $ini = $dath['ini'];
                                            $fin = $dath['fin'];
                                            $tin = ($dath['tin'] == "" || $dath['tin'] == NULL || $dath['tin'] == 0) ? 1 : $dath['tin'];
                                            $obs = br2nl($dath['obs']);
                                            ...
                                            <tr id="rowh_<?php echo $joh;?>" class=""> 
                                                ...
                                                <!-- Total Horas -->
                                                <td class="p-1 text-center" style="width:10%">
                                                    <span id="spantot<?php echo $joh;?>"><?php echo $tot;?></span>
                                                </td>
                                                ...
                                                <td class="p-1" style="width:20px">
                                                    <a class="btn btn-sm bg-red text-white <?php echo $visbot;?>" onclick="CRUDJORNADA('ELIMINARHORA','<?php echo $joh;?>','<?php echo $idfec;?>')">
    ...
    $stmtHor = $conn->prepare("
        SELECT joh_clave_int AS id, obr_clave_int AS obr, 
               DATE_FORMAT(joh_inicio,'%H:%i') AS ini, 
               DATE_FORMAT(joh_fin,'%H:%i') AS fin, 
               tin_clave_int AS tin, joh_observacion AS obs 
        FROM tbl_jornada_horas 
    ...
    if ($numhor <= 0) {
        ...
        foreach ($horas as $nh => $dath) {
            ...
            <tr id="rowh_<?php echo $joh; ?>">
                ...
                <td class="p-1" style="width:20px">
                    <a class="btn btn-sm rounded-circle bg-red text-white" onclick="CRUDJORNADA('ELIMINARHORA','<?php echo $joh; ?>','<?php echo $idfec; ?>')">
                        <i class="fas fa-trash"></i>
                    </a>
                </td>
            </tr>
    ...
    $stmtUpd = $conn->prepare("
        UPDATE tbl_jornada_dias 
        SET tin_clave_int = :nov, 
            jod_usu_actualiz = :usuario, 
            jod_fec_actualiz = :fecha 
        WHERE jod_clave_int = :id
    ");

    $updSuccess = $stmtUpd->execute([
        ':nov' => $nov,
        ':usuario' => $usuario,
        ':fecha' => $fecha,
        ':id' => $id
    ...
    if ($updSuccess) {
        ...
        $reg = new General();
        $datemp = $reg->fechaDia($id); // devuelve ['emp' => ..., 'fec' => ...]
        $emp = $datemp['emp'];
        $fec = $datemp['fec'];
        ...
        // Obtener obras de ese día para recalcular
        $stmtHoras = $conn->prepare("SELECT obr_clave_int AS obr FROM tbl_jornada_horas WHERE jod_clave_int = :id");
        $stmtHoras->execute([':id' => $id]);
        $horas = $stmtHoras->fetchAll(PDO::FETCH_ASSOC);

        if (!empty($horas)) {
            foreach ($horas as $dath) {
                $obr = $dath['obr'];
        ...
        $res = "error";
        $msn = "Surgió un error al actualizar día con la novedad. Verificar";
    }
    ...
    // Insertar nueva hora vacía
    $stmt = $conn->prepare("
        INSERT INTO tbl_jornada_horas (jod_clave_int, joh_usu_actualiz, joh_fec_actualiz)
        VALUES (:id, :usuario, :fecha)
    ");

    $insSuccess = $stmt->execute([
        ':id' => $id,
        ':usuario' => $usuario,
        ':fecha' => $fecha
    ]);
    ...
    if ($insSuccess) {
        ...
        // Si necesitas el ID insertado:
        // $nuevoID = $conn->lastInsertId();
    } else {
        $res = "error";
        $msn = "Surgió un error al agregar la hora";
    }
    ...
    // Eliminar la hora
    $stmt = $conn->prepare("DELETE FROM tbl_jornada_horas WHERE joh_clave_int = :id");
    ...
    if ($success) {
        ...
        $res = "error";
        $msn = "Surgió un error al eliminar la hora";
    }
    ...
    $datob = $reg->datosHora($joh); // obtiene datos anteriores (ej: obra previa)
    $obrant = $datob['obr'];

    // Actualización con parámetros bind
    $stmt = $conn->prepare("
        UPDATE tbl_jornada_horas 
        SET obr_clave_int = :obr,
            joh_inicio = :ini,
            joh_fin = :fin,
            tin_clave_int = :tin,
            joh_observacion = :obs,
            joh_usu_actualiz = :usuario,
            joh_fec_actualiz = :fecha
        WHERE joh_clave_int = :joh
    ");

    $updSuccess = $stmt->execute([
        ':obr' => $obr,
        ':ini' => $ini,
        ':fin' => $fin,
        ':tin' => $tin,
    ...
    if ($updSuccess) {
        ...
        // Recalcular por obra actual
        if ($obr > 0) {
            $msn .= $reg->calcularDiaObra($emp, $fec, $obr, $usuario, $fecha);

            // Si la obra cambió, recalcular también la anterior
            if ($obr != $obrant && $obrant > 0) {
                $msn .= $reg->calcularDiaObra($emp, $fec, $obrant, $usuario, $fecha);
            }
        }
    } else {
        $res = "error";
        $msn = "Surgió un error al actualizar hora. Verificar";
    }
    ...
    // Total de horas ordinarias por semana
    $stmtTot = $conn->prepare("
        ...
        WHERE tn.tin_tipo = 1 AND jd.usu_clave_int = :emp AND jd.jod_semana = :sem
    ");
    $stmtTot->execute([':emp' => $emp, ':sem' => $sem]);
    $dattot = $stmtTot->fetch(PDO::FETCH_ASSOC);

    $totalhoras = $dattot['totalhoras'];
    $totalhoras = (empty($totalhoras)) ? 0 : $totalhoras;

    // Variables iniciales para datos del día específico
    $dia = "";
    $fes = 0;
    $totalhorasdia = 0;

    if ($idfec > 0) {
        // Consulta detallada del día
        $stmtDia = $conn->prepare("
            SELECT 
                SUM(calculoHoras3(jd.jod_fecha, joh_inicio, joh_fin)) AS totalhoras,
                jod_fecha AS dia,
                diaHabil(jod_fecha) AS fes,
                jd.jod_clave_int AS id
            FROM tbl_jornada_dias jd
            JOIN tbl_jornada_horas jh ON jh.jod_clave_int = jd.jod_clave_int
            JOIN tbl_tipo_novedad tn ON tn.tin_clave_int = jh.tin_clave_int
            WHERE tn.tin_tipo = 1 AND jd.jod_clave_int = :idfec
            GROUP BY jd.jod_clave_int
        ");
        $stmtDia->execute([':idfec' => $idfec]);
        $dattotd = $stmtDia->fetch(PDO::FETCH_ASSOC);

        $totalhorasdia = (empty($dattotd['totalhoras'])) ? 0 : $dattotd['totalhoras'];
        $dia = $dattotd['dia'];
        $fes = $dattotd['fes'];

        // Actualizar total de horas del día
        $stmtUpd = $conn->prepare("UPDATE tbl_jornada_dias SET jod_total = :total WHERE jod_clave_int = :idfec");
        $stmtUpd->execute([':total' => $totalhorasdia, ':idfec' => $idfec]);
    }
...
else if($opcion=="ACTUALIZARHORAOLD")
{
    ...
    // Verificar si ya existe la jornada
    $stmtVeri = $conn->prepare("SELECT jor_clave_int, jor_estado FROM tbl_jornada WHERE usu_clave_int = :emp AND jor_semana = :sem");
    $stmtVeri->execute([':emp' => $emp, ':sem' => $sem]);
    $num = $stmtVeri->rowCount();

    if ($num > 0) {
        $dat = $stmtVeri->fetch(PDO::FETCH_ASSOC);
        ...
        $stmtUpd = $conn->prepare("
            UPDATE tbl_jornada 
            SET jor_total = :tot, 
                jor_usu_actualiz = :usuario, 
                jor_fec_actualiz = :fecha, 
                jor_nota = :nota 
            WHERE jor_clave_int = :idjor
        ");
        $upd = $stmtUpd->execute([
            ':tot' => $tot,
            ':usuario' => $usuario,
        ...
        $stmtIns = $conn->prepare("
            INSERT INTO tbl_jornada (
                jor_fec_creacion, jor_creacion, usu_clave_int, 
                jor_semana, jor_ano, jor_nota, 
                jor_usu_actualiz, jor_fec_actualiz, jor_total
            ) VALUES (
                :fec, :creador, :emp, :sem, :ano, :nota, :usuario, :fecact, :tot
            )
        ");
        $ins = $stmtIns->execute([
            ':fec'     => $fecha,
        ...
        if ($ins) {
            $idjor = $conn->lastInsertId();

            // Asociar los días de la semana al ID de la jornada recién insertada
            $stmtUpdDias = $conn->prepare("
                UPDATE tbl_jornada_dias 
                SET jor_clave_int = :idjor 
                WHERE usu_clave_int = :emp AND jod_semana = :sem
            ");
            $stmtUpdDias->execute([':idjor' => $idjor, ':emp' => $emp, ':sem' => $sem]);
    ...
    // Aquí puedes seguir con la lógica de recalcular jornada_dias y obra si lo necesitas
...
else if($opcion=="REGISTROMASIVO")
{
    ...
    SELECT jod_clave_int, jod_fecha 
    FROM tbl_jornada_dias 
    WHERE usu_clave_int = :emp AND jod_semana = :sem AND jod_fecha <= CURDATE()");
    $stmtDias->execute([':emp' => $emp, ':sem' => $sem]);
    ...
    $placeholders = implode(',', array_fill(0, count($dias), '?')); // para usar con IN
    $params = $dias;

    if ($hi == "" || $hf == "") {
        // Sólo se asigna la novedad, sin insertar horarios
        $sql = "UPDATE tbl_jornada_dias 
                SET tin_clave_int = ?, jod_usu_actualiz = ?, jod_fec_actualiz = ?
                WHERE jod_clave_int IN ($placeholders)";
        $stmt = $conn->prepare($sql);
        $exec = $stmt->execute(array_merge([$nov, $usuario, $fecha], $params));

        if ($exec) {
            $res = "ok";
            $msn = "Días seleccionados modificados con la novedad seleccionada";
        } else {
            $msn = "Surgió un error al modificar los días seleccionados. Verificar";
        }
    } else {
        // Inserción masiva de horas usando SELECT
        $sql = "
            INSERT INTO tbl_jornada_horas (
                jod_clave_int, obr_clave_int, joh_inicio, joh_fin, 
                tin_clave_int, joh_observacion, joh_usu_actualiz, joh_fec_actualiz
            )
            SELECT jod_clave_int, ?, ?, ?, ?, ?, ?, ?
            FROM tbl_jornada_dias
            WHERE jod_clave_int IN ($placeholders)
        ";
        $stmt = $conn->prepare($sql);
        $exec = $stmt->execute(array_merge([$obr, $hi, $hf, $nov, $obs, $usuario, $fecha], $params));

        if ($exec) {
            $res = "ok";
            $msn = "Jornada registrada correctamente a los días seleccionados";
        } else {
            $msn = "Surgió un error al guardar. Verificar";
        }
    }

    // Recalcular horas por día y actualizar totales si fue exitoso
    if ($res == "ok") {
        $reg = new General();

        foreach ($_POST['dias'] as $id) {
            $datemp = $reg->fechaDia($id);
            $fec = $datemp['fec'];

            // Calcular total de horas del día
            $stmtTotDia = $conn->prepare("
                SELECT SUM(calculoHoras3(jd.jod_fecha, joh_inicio, joh_fin)) AS totalhoras
                FROM tbl_jornada_dias jd
                JOIN tbl_jornada_horas jh ON jh.jod_clave_int = jd.jod_clave_int
                JOIN tbl_tipo_novedad tn ON tn.tin_clave_int = jh.tin_clave_int
                WHERE tn.tin_tipo = 1 AND jd.jod_clave_int = ?
                GROUP BY jd.jod_clave_int
            ");
            $stmtTotDia->execute([$id]);
            $dattotd = $stmtTotDia->fetch(PDO::FETCH_ASSOC);

            $totalhorasdia = (empty($dattotd['totalhoras'])) ? 0 : $dattotd['totalhoras'];

            // Actualizar total en jornada_dias
            $stmtUpd = $conn->prepare("UPDATE tbl_jornada_dias SET jod_total = ? WHERE jod_clave_int = ?");
            $stmtUpd->execute([$totalhorasdia, $id]);

            // Recalcular jornada y obra
            $msn .= "Fec:" . $fec;
            $msn .= $reg->calcularDiaLiquidar($emp, $fec, $usuario, $fecha);
            $msn .= "<br>" . $reg->calcularDiaObra($emp, $fec, $obr, $usuario, $fecha);
        }
    ...
    // Para referencia (útil en debug)
    $sqlDebug = "SELECT jod_clave_int, '$obr', '$hi', '$hf', '$nov', '$obs', '$usuario', '$fecha' FROM tbl_jornada_dias WHERE jod_clave_int IN ($placeholders)";
    $datos[] = array("res" => $res, "msn" => $msn, "debug" => $sqlDebug);
