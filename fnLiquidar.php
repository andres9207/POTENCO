...
if($opcion=="NUEVO" || $opcion=="EDITAR" || $opcion=="APPROVED")
{
    ...
    $conliq = mysqli_query($conectar, "SELECT l.usu_clave_int usu,l.obr_clave_int obr, l.liq_ano ano, l.liq_inicio ini, l.liq_fin fin, l.liq_tipo tip,l.liq_estado est,l.liq_hr_mes hor, l.liq_salario sal, l.liq_total tot, liq_notas nota from tbl_liquidar l WHERE l.liq_clavE_int = '".$id."'");
    $datliq = mysqli_fetch_array($conliq);
    $emp   = $datliq['usu'];
    $obr   = $datliq['obr'];
            ...
            <tbody>
                ...
                {                    
                    for($d=0;$d<$numdias;$d++)
                    {
                        ...
                        $condatosdia = mysqli_query($conectar, "SELECT lid_clave_int, lid_fecha fec, lid_hi_man, lid_hf_man,lid_hi_tar, lid_hf_tar, lid_horas, lid_horario,lid_hedo, lid_hdf,lid_hedf, lid_heno, lid_henf, lid_rn, lid_hnf, lid_rd, lid_observacion, lid_alimentacion, lid_val_alimentacion, lid_estado, lid_compensado, lid_remunerado,lid_procede, lid_auxilio, lid_bonificacion, lid_labores, lid_val_bonificacion, lid_auxilio, lid_permisos FROM tbl_liquidar_dias WHERE lid_fecha = '".$fec."' and usu_clave_int = '".$emp."' limit 1");
                        $datdia = mysqli_fetch_array($condatosdia);
                        ...
                        $tothoras = $totalhoras!=null ? str_replace(",",".",$tothoras) : 0;
                        ...
                        <tr class="<?php echo $bgfes;?> <?PHP echo $CE;?>" id="rowfec_<?php echo $iddia;?>" <?php echo $ME;?>>
                            ...
                            <td class="p-0 text-center align-middle" style="width:100px; cursor:pointer" title="Doble clic para realizar cambios <?php echo $fecha; ?>" ondblclick="CRUDJORNADA('EDITARJORNADA','<?php echo $jor;?>')"><?php echo $fec;?></td>
                        ...
                        {
                            $conld = mysqli_query($conectar,"SELECT STR_TO_DATE('".$sem." Monday', '%x%v %W') LUN,STR_TO_DATE('".$sem." Sunday', '%x%v %W') DOM ");
                            $datld = mysqli_fetch_array($conld);
                            $sLunes = $datld['LUN'];
                            $sDomingo = $datld['DOM'];
                            $sqlhorasem = mysqli_query($conectar,"select sum(lid_hedo) tothedo,sum(lid_hdf) tothdf,sum(lid_hedf) tothedf,sum(lid_heno) totheno,sum(lid_henf) tothenf,sum(lid_henf) tothenf,sum(lid_rn) totrn, sum(lid_hnf) tothnf, sum(lid_rd) totrd, sum(lid_horas) totalhoras, sum(lid_permisos) totpermisos, sum(lid_val_bonificacion) totbon, sum(CASE WHEN lid_procede=1 and lid_alimentacion=1 THEN lid_val_alimentacion ELSE 0 END) totali,  sum(lid_auxilio) totaux  from tbl_liquidar_dias where  usu_clave_int = '".$emp."' and lid_fecha between '".$sLunes."' and '".$sDomingo."'");
                            $datsem = mysqli_fetch_array($sqlhorasem);
            ...
            <tbody>
                ...
                if($numdias>0)
                {
                    ...
                    for($d=0;$d<$numdias;$d++)
                    {
                        ...
                        $condatosdia = mysqli_query($conectar, "SELECT lid_clave_int, lid_fecha fec, lid_hi_man, lid_hf_man,lid_hi_tar, lid_hf_tar, lid_horas,lid_horas_maquina, lid_horario,lid_hedo, lid_hdf,lid_hedf, lid_heno, lid_henf, lid_rn, lid_hnf, lid_rd, lid_observacion, lid_alimentacion, lid_val_alimentacion, lid_estado, lid_compensado, lid_remunerado,lid_procede, lid_auxilio, lid_bonificacion, lid_labores, lid_val_bonificacion FROM tbl_liquidar_dias_obra WHERE lid_fecha = '".$fec."' and usu_clave_int = '".$emp."' and obr_clave_int = '".$obr."' limit 1");
                        $datdia = mysqli_fetch_array($condatosdia);
                        $iddia = $datdia['lid_clave_int'];
                        ...
                        <tr class="<?php echo $bgfes;?> <?PHP echo $CE;?>" id="rowfec_<?php echo $iddia;?>" <?PHP echo $ME;?>>
                            ...
                            <td class="p-0 text-center align-middle" style="width:100px; cursor:pointer" title="Doble clic para realizar cambios" ondblclick="CRUDJORNADA('EDITARJORNADA','<?php echo $jor;?>')"><?php echo $fec;?></td>
                            <td class="text-center p-0 align-middle"><?php echo $aminit;?></td>
                            <td class="text-center p-0 align-middle">a</td>
...
else if($opcion=="CARGAREMPLEADO")
{
    ...
    {
        $con = mysqli_query($conectar,"SELECT DISTINCT u.usu_clave_int id, concat(usu_apellido,' ',usu_nombre) nom, usu_correo cor from tbl_usuarios u join tbl_jornada j on j.usu_clave_int  = u.usu_clave_int join tbl_jornada_dias d on d.jor_clave_int = j.jor_clave_int join tbl_jornada_horas h on h.jod_clave_int = d.jod_clave_int WHERE (u.est_clave_int = 1 ) and  j.jor_estado = 1 and d.jod_fecha between '".$ini."' and '".$fin."'  and h.obr_clave_int = '".$obr."' ORDER BY usu_apellido ASC"); // or usu_clave_int = '".$sel."'
        $empleados = array();
...
else if($opcion=="GENERARALLPROFORMA")
{
    $id = $_POST['id'];
    $tip = $_POST['tip'];
    // $emp = $_POST['emp'];    
...
else if($opcion=="UPDATEPROCEDE")
{
    $id = $_POST['id'];
    $pro = $_POST['pro'];
    $sqlu = "UPDATE tbl_liquidar_dias_obra SET lid_procede = '".$pro."', lid_usu_actualiz = '".$usuario."', lid_fec_actualiz = '".$fecha."' WHERE lid_clave_int = '".$id."'";
    $upd = mysqli_query($conectar,$sqlu);
    if($upd>0)
    {
        $res = "ok";
...
else if($opcion=="UPDATEAUX")
{
    $id = $_POST['id'];
    $aux = $_POST['aux'];
    $tip = $_POST['tip'];// 1 LIQUIDAR NOMI 2 LIQUIDAR OBRA
    if($tip==1)
    {
        $sqlu = "UPDATE tbl_liquidar_dias SET lid_auxilio = '".$aux."',  lid_usu_actualiz = '".$usuario."', lid_fec_actualiz = '".$fecha."' WHERE lid_clave_int = '".$id."'";
    }
    else
    {
        $sqlu = "UPDATE tbl_liquidar_dias_obra SET lid_auxilio = '".$aux."', lid_usu_actualiz = '".$usuario."', lid_fec_actualiz = '".$fecha."' WHERE lid_clave_int = '".$id."'";
    }
    $upd = mysqli_query($conectar,$sqlu);
    if($upd>0)
    {
    ...
    {
        $res = "error";
        $msn = "Surgió un error al modificar auxilio";
    }
    $datos[] = array("res"=>$res, "msn"=>$msn);
    echo json_encode($datos);
}
else if($opcion=="UPDATEBON")
{
    ...
    if($tip==1)
    {
        $sqlu = "UPDATE tbl_liquidar_dias SET lid_val_bonificacion = '".$bon."', lid_usu_actualiz = '".$usuario."', lid_fec_actualiz = '".$fecha."' WHERE lid_clave_int = '".$id."'";
    }
    else
    {
        $sqlu = "UPDATE tbl_liquidar_dias_obra SET lid_val_bonificacion = '".$bon."',lid_usu_actualiz = '".$usuario."', lid_fec_actualiz = '".$fecha."' WHERE lid_clave_int = '".$id."'";
    }
    $upd = mysqli_query($conectar,$sqlu);
    if($upd>0)
    {
    ...
    {
        $res = "error";
        $msn = "Surgió error al modificar bonificacion";
    }
    $datos[] = array("res"=>$res, "msn"=>$msn);
    echo json_encode($datos);
}
...
else if($opcion=="CARGAREMPLEADOSLIQUIDAR")
{
    ...
    $tipo = $_POST['tip'];
    $obr  = $_POST['obr'];
    $wh = ($tipo==2)?"and h.obr_clave_int = '".$obr."'":"";
    $con = mysqli_query($conectar,"SELECT DISTINCT u.usu_clave_int id, concat(usu_apellido,' ',usu_nombre) nom, usu_correo cor from tbl_usuarios u join tbl_jornada j on j.usu_clave_int  = u.usu_clave_int join tbl_jornada_dias d on d.jor_clave_int = j.jor_clave_int join tbl_jornada_horas h on h.jod_clave_int = d.jod_clave_int WHERE (u.est_clave_int = 1) and  j.jor_estado = 1 and d.jod_fecha between '".$ini."' and '".$fin."'".$wh." ORDER BY usu_apellido ASC"); // or usu_clave_int = '".$sel."'
    $empleados = array();
    while($dat = mysqli_fetch_array($con))
    {
        $id  = $dat['id'];
...
else if($opcion=="VEREMPLEADOSLIQUIDAR")
{
    ...
    $con = mysqli_query($conectar,"SELECT DISTINCT u.usu_clave_int id, concat(usu_apellido,' ',usu_nombre) nom, usu_correo cor from tbl_usuarios u join tbl_jornada j on j.usu_clave_int  = u.usu_clave_int join tbl_jornada_dias d on d.jor_clave_int = j.jor_clave_int join tbl_jornada_horas h on h.jod_clave_int = d.jod_clave_int WHERE (u.est_clave_int = 1 ) and  j.jor_estado = 1 and d.jod_fecha between '".$ini."' and '".$fin."' and (j.usu_clave_int  in(".$emp1.")  OR '".$emp."' IS NULL OR '".$emp."' = '') ".$wh. " ORDER BY usu_apellido ASC"); // or usu_clave_int = '".$sel."'
    $num = mysqli_num_rows($con);

    //echo "SELECT DISTINCT u.usu_clave_int id, concat(usu_nombre,' ',usu_apellido) nom, usu_correo cor from tbl_usuarios u join tbl_jornada j on j.usu_clave_int  = u.usu_clave_int join tbl_jornada_dias d on d.jor_clave_int = j.jor_clave_int join tbl_jornada_horas h on h.jod_clave_int = d.jod_clave_int WHERE (u.est_clave_int = 1 and u.prf_clave_int = 2) and  j.jor_estado = 1 and d.jod_fecha between '".$ini."' and '".$fin."' and (j.usu_clave_int  in(".$emp1.")  OR '".$emp."' IS NULL OR '".$emp."' = '') ".$wh;
    
    $empleados = array();
   
    //echo $emp;
    if($num>0 and (($tipo==1 and $emp!="") || ($tipo==2))){ 
    ?>
    <div id="smartwizard" class="mt-2">
        <ul class="nav">
            ...
            while($dat = mysqli_fetch_array($con))
            {
                ...
                //VALIDAR A CUALES SE LE GENERO LIQUIDACION A ESE EMPLEADO EN EL RANGO DE FECHA SELECCIONADO
                $sqlva = mysqli_query($conectar,"SELECT distinct l.liq_clave_int liquidacion, l.liq_estado estado FROM tbl_liquidar_dias ld join tbl_liquidar l on l.liq_clave_int = ld.liq_clave_int WHERE ld.usu_clave_int = '".$id."' and l.liq_inicio  = '".$ini."' and l.liq_fin =  '".$fin."'");
                $numva = mysqli_num_rows($sqlva); // VALIDAR QUE SI TENGA UNA LIQUIDACION YA GENERADA EN EL RANGO SELECCIONADO

                // VALIDAR QUE HAYAN DIAS DENTRO DEL RANGO SELECCION

                // SI NUMVA ES MENOR A CERO PERO SI LA BUSQUEDA NUMHD ES MAYOR A CERO ES POR QUE ESTA LIQUIDACION DIAS  
                
                $empleados[] = $id;
                ?>
                    <li title="<?php echo $numva;?>"><a class="nav-link <?php if($numva>0){ echo "generate"; } ?>" data-empleado = "<?php echo $id;?>" data-target="#step-<?php echo $i;?>"><?php echo $nom;?></a></li>
                <?php
                $i++;
            }
            ?>
        ...
        <div class="tab-content">
            <?php 
            for($k=0;$k<count($empleados);$k++){ ?>
                <div id="step-<?php echo $k;?>" data-empleado="<?php echo $empleados[$k]; ?>" class="tab-pane invoice table-responsive" role="tabpanel"><?php echo $k;?></div>  
            <?php } ?>          
        </div>
    </div>
    <script>
        ...
        $("#smartwizard").on("stepContent", function(e, anchorObject, stepIndex, stepDirection) {
            ...
            //var o = (stepIndex==0)?"VERDETALLE": (stepIndex==1)?"VERCONTRATOADMON": (stepIndex==2)?"VERPERIODOSADMON": (stepIndex==3)?"VERCONTRATOARRENDAMIENTO": (stepIndex==4)?"VERPERIODOARRENDAMIENTO":'';
            //elmForm.html(o)                   
            // infoDetalle(elmForm,o,d.Id);  
            //$("#smartwizard"+ d.Id).smartWizard("loader", "show");     
        });
    </script>
    <?php
        echo "<script>CRUDLIQUIDAR('VERDETALLELIQUIDAR','".$idinicial."','0');</script>";
    }
    else
    {
        ?>
...
else if($opcion=="LIQUIDAROBRA")
{
    ...
    $conhoras = mysqli_query($conectar, $sqlhoras);
        ...
        <div class="col-md-12">
            <table  id="tbLiquidacionesObra" class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    while($dath = mysqli_fetch_array($conhoras)){ 
...
else if($opcion=="GUARDARLIQUIDAROBRA")
{
    ...
    $ins = mysqli_query($conectar, "INSERT INTO tbl_liquidar_obras(obr_clave_int, lio_horas_maquina, lio_horas_contrato, lio_horas_extras, lio_vr_hora_maquina, lio_vr_hora, lio_inicio, lio_fin, usu_liquidador, lio_fec_creacion, lio_usu_actualiz, lio_fec_actualiz, lio_observacion) VALUES('".$obr."','".$horasmaquina."','".$horascontrato."','".$extras."','".$vrhoramaquina."','".$vrhoraempleado."','".$ini."','".$fin."','".$idUsuario."','".$fecha."','".$usuario."','".$fecha."','".$obs."')");
    if($ins>0)
    {
        $idli = mysqli_insert_id($conectar);
        $codigo = sprintf('%010d',$idli);
        //ACTUALIZAR LAS LIQUIDACION OBRA DE EMPLEADO CON EL ID DE LA LIQUIDACION DE OBRA
        $updliquidacion = mysqli_query($conectar, "UPDATE tbl_liquidar SET lio_clave_int = '".$idli."' WHERE liq_clave_int IN(".$liquidaciones.")");
        if($updliquidacion>0)
        {
            //ACTUALIZAR LA LIQUIDACION CON EL CODIGO GENERADO
            $updliquidar= mysqli_query($conectar, "UPDATE tbl_liquidar_obras SET lio_codigo = '".$codigo."' WHERE lio_clave_int = '".$idli."'");

            //INSERTAR CON QUE PORCENTAJES DE HORAS SE LIQUIDO YA QUE CON EL TIEMPO PUEDES VARIAR LOS PORCENTAHES
            $inshora = mysqli_query($conectar, "INSERT INTO tbl_liquidar_obras_horas(lio_clave_int, hor_clave_int,lih_porcentaje) SELECT '".$idli."', hor_clave_int, hor_porcentaje FROM tbl_horas WHERE est_clave_int = 1");

            $res = "ok";
            $msn = "Liquidación de Obra con  codigo N°".$codigo. " generada correctamente";
        }
        else
        {
            $res = "error";
            $msn = "Surgió un error al asociar la liquidación a los empleados seleccionados";
        }
    }
    else
    {
        $res = "error";
        $msn = "Surgió un error al generar liquidación de obra. Verificar";
    }
    $datos[] = array("res"=>$res, "msn"=>$msn);
    echo json_encode($datos);
}